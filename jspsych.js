...
(function($) {
	jsPsych = (function() {
		...
		core.init = function(options) {
			...
			exp_chunks = parseExpStructure(opts.experiment_structure);
		...
		core.currentChunkID = function(){
			return exp_chunks[curr_chunk].currentTrialChunkID();
		}
		...
		function createExperimentChunk(chunk_definition, chunk_id){
			
			var chunk = {};
			
			chunk.timeline = parseChunkDefinition(chunk_definition.blocks);
			
			chunk.type = chunk_definition.chunk_type; // linear, while
			
			chunk.id = chunk_id; // some unique identifier for this particular chunk
			...
			chunk.currentTrialChunkID = function() {
				
				if(typeof this.timeline[this.currentTimelineLocation].id !== 'undefined'){
					return this.timeline[this.currentTimelineLocation].currentTrialChunkID();
				} else {
			...
			chunk.generatedData = function() {
				// return an array containing all of the data generated by this chunk
				var d = jsPsych.data.getTrialsFromChunk(this.currentTrialChunkID());
				return d;
			}
			...
			function parseChunkDefinition(chunk_timeline){
				...
				for (var i = 0; i < chunk_timeline.length; i++) {
					// if this is a nested chunk, then create a new chunk for this spot
					if(typeof chunk_timeline[i].chunk_type !== 'undefined') {
						timeline.push(createExperimentChunk(chunk_timeline[i], chunk_id + "." + i))
					} else {
						...
						var trials = jsPsych[plugin_name]["create"].call(null, chunk_timeline[i]);
	...
	jsPsych.data = (function() {
		...
		module.getTrialsFromChunk = function(chunk_id) {
			...
			var trials = [];
			for (var i = 0; i < data.length; i++) {
				if (data[i].internal_chunk_id === chunk_id) {
					trials.push(data[i]);
				}
			}
