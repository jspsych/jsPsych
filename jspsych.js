...
(function($) {
	jsPsych = (function() {
		...
		core.currentChunkID = function(){
			return exp_chunks[curr_chunk].id;
		}
		...
		function createExperimentChunk(chunk_definition, chunk_id){
			...
			chunk.blocks = convertBlocksToTrials(chunk_definition.blocks);
			...
			chunk.generatedData = function() {
				...
				var d = jsPsych.data.getTrialsFromChunk(this.id);
				return d;
			}
			...
			function convertBlocksToTrials(chunk_blocks){
				
				var blocks = [];
				
				for (var i = 0; i < chunk_blocks.length; i++) {

					// check to make sure plugin is loaded
					var plugin_name = chunk_blocks[i].type;
					...
					var trials = jsPsych[plugin_name]["create"].call(null, chunk_blocks[i]);

					// add options that are generic to all plugins
					trials = addGenericTrialOptions(trials, chunk_blocks[i]);

					blocks.push(createBlock(trials));
				}
