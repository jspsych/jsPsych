...
filename | string \| function | *undefined* | The name of the file to save. Every file in an experiment must have a unique name. Required. This can be a function if you are generating a subject ID that is not known at the moment of initializing jsPsych.
...
This is different from most extensions, which run per-trial callbacks that fire only for trials whose own `extensions` parameter names them.
...
It **throws an error** if the condition cannot be obtained. There is no safe value to fall back to: a participant sent down the wrong branch, or an empty one, looks like a successful run until someone reads the data. Decide what they should see.
