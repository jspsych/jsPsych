...
* `survey` plugin
    ...
    * Cannot be used with jsPsych's timeline variables
...
SurveyJS allows you to build surveys using a JSON string, a JavaScript function, or a combination of both. You can read more about these options in the SurveyJS documentation:
...
SurveyJS allows you to define the survey contents using JSON. At a minimum, the survey JSON object should contain a property called 'elements'. The value of 'elements' is an array that contains at least one element/question to be shown on the page.
...
!!! note "JSON vs JavaScript objects"
    ...
    [JSON (JavaScript Object Notation)](https://developer.mozilla.org/en-US/docs/Learn/JavaScript/Objects/JSON) is a text format for organizing data. It is very similar to a JavaScript object, but not exactly the same. In our examples, the survey JSON is created using JavaScript objects that are then converted to JSON using `JSON.stringify()` before being passed into the `survey_json` parameter. You can also write the survey configuration directly as JSON and skip the conversion step. To read more about JSON vs JavaScript objects, see e.g. [here](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON#javascript_and_json_differences) and [here](https://www.w3schools.com/js/js_json_intro.asp).
...
const color_survey_trial = {
  type: jsPsychSurvey,
  survey_json: JSON.stringify(color_survey_json),
...
### Deciding between JSON and JavaScript
...
But another option is to use the `survey_function` to loop over your question prompts and dynamically add questions to your survey. This way you can separate the information that changes across questions from the things that are common across all questions, which can make it easier to make changes and prevent errors (compared to working with very large and repetitive JSON structures).
