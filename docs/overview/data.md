...
```js
var data = jsPsych.data.get().filter({trial_type: imageKeyboardResponse});
```
...
```js
var data = jsPsych.data.get().filter({trial_type: imageKeyboardResponse}).filterCustom(function(x){ return x.rt > 100; });
```
