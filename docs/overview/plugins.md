...
| multiplayer_scope | string | *undefined* | For [multiplayer](../reference/jspsych-multiplayer.md#scopes) experiments: names the part of the shared data this trial reads and writes. By default, each trial gets its own part, named after its position in the timeline. The name is used exactly as written, so trials with the same name share data; a trial that repeats needs a name that changes with each repetition, e.g., one built from a timeline variable. |
