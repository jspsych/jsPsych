...
```html
<head>
  <script src="https://unpkg.com/jspsych@8.3.0" type="text/javascript"></script>
  <script src="https://unpkg.com/@jspsych/plugin-image-keyboard-response@2.2.0" type="text/javascript"></script>
</head>
```
...
| run_on_resume | boolean | `false` | Only has an effect when the [resume option](../overview/resume.md) is used. If set to `true`, then this trial will be run again when a saved session is resumed, instead of having its saved data restored. This is useful for trials that establish a state that a page reload destroys, such as entering fullscreen mode or initializing a camera. See [the Resuming After a Page Reload page](../overview/resume.md#re-running-a-trial-after-a-reload-run_on_resume) for more details. |
