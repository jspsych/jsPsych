...
override_safe_mode | boolean | If jsPsych detects that the HTML file has been opened directly in the browser, rather than running via a server, then it will disable Web Audio and video preloading in order to prevent cross-origin request (CORS) errors. Default is `false`. If `true`, jsPsych will ignore the file's protocol and use the normal settings for Web Audio (`use_webaudio`) and video preloading (automatic preloading via video plugins and/or manual preloading via `preload_videos`).
