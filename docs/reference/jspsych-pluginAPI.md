...
release_callback_function | function | An optional function that is called when the response key is released (on `keyup`). It receives a single object argument with the properties `key` (the string representation of the released key) and `duration` (the time in milliseconds that the key was held down, measured from key press to key release, or `null` if no press timestamp was recorded). The release callback is registered when a valid response triggers `callback_function`, and it will still fire even if the keyboard listener has already been cancelled (e.g. because the trial ended at the key press). Duration is always measured with `performance.now()`, regardless of `rt_method`.
