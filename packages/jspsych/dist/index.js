...
function copy(data) {
  return data === void 0 ? data : JSON.parse(JSON.stringify(data));
}
class MultiplayerAPI {
  constructor() {
     ...
     */
    this.activeUnsubscribes = /* @__PURE__ */ new Set();
  ...
  async connect(adapter) {
    if (this.adapter || this.connectingAdapter) {
      throw new Error(
        "MultiplayerAPI: connect() has already been called. Call disconnect() first before registering a new adapter."
      );
    }
  ...
  async pushPendingUpdates(epoch) {
    try {
...
export { DataCollection, JsPsych, MultiplayerCancelledError, MultiplayerTimeoutError, ParameterType, initJsPsych };
